                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ route('admin.blogs.store') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                ...
                                <input id="title" class="form-control @error('title') is-invalid @enderror" name="title" type="text" value="{{ old('title') }}" required>
                                @error('title')
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control @error('excerpt') is-invalid @enderror" name="excerpt" id="excerpt" rows="3" required>{{ old('excerpt') }}</textarea>
                                @error('excerpt')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input id="meta_title" class="form-control @error('meta_title') is-invalid @enderror" name="meta_title" type="text" value="{{ old('meta_title') }}">
                                @error('meta_title')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control @error('meta_description') is-invalid @enderror" name="meta_description" id="meta_description" rows="3">{{ old('meta_description') }}</textarea>
                                ...
                                <div class="card-body">
                                        ...
                                        <div class="col-md-6">
                                            <div class="mb-3">
                                                ...
                                                <div class="d-flex justify-content-between">
                                                    <span>Title: <span id="title-count">0</span>/60</span>
                                                    <span>Description: <span id="desc-count">0</span>/160</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div id="seo-recommendations" class="mt-3">
                                        ...
                                        <ul id="recommendations-list" class="list-unstyled">
                                            <li class="text-muted">Start typing to see SEO recommendations...</li>
                                        </ul>
                                    </div>
...
function testAuth() {
    ...
    fetch('{{ route("admin.blogs.test-auth") }}')
...
// Test SEO route
function testSeoRoute() {
    console.log('Testing SEO route...');
    ...
    .then(response => {
        console.log('SEO route test response status:', response.status);
        return response.json();
    })
    .then(data => {
        console.log('SEO route test response:', data);
        alert('SEO route test: ' + JSON.stringify(data, null, 2));
    })
    .catch(error => {
        console.error('SEO route test error:', error);
        alert('SEO route test error: ' + error.message);
    });
}
...
function analyzeSEO() {
    ...
    const seoRoute = '{{ route("admin.blogs.seo-suggestions") }}';
    console.log('SEO route generated:', seoRoute);
...
function setupAutoSave() {
    ...
    [title, content, excerpt].forEach(field => {
        field.addEventListener('input', () => {
            clearTimeout(autoSaveTimer);
            autoSaveTimer = setTimeout(() => {
                // You can implement auto-save to localStorage or make AJAX call here
